...
const source = (slug: string) => `https://reactbits.dev/backgrounds/${slug}`;
const effect = (name: string, slug: string, description: string, engine: ParticleEngine, dependencies: string[], componentImport: ParticleEffect['componentImport']): ParticleEffect => ({
...
export const particleEffects: readonly ParticleEffect[] = [
...
export const getParticleEffect = (slug: string) => particleEffects.find(effect => effect.slug === slug);
/** Lazily loads a single effect so galleries never create every WebGL context. */
export const lazyParticleEffect = (effect: ParticleEffect): LazyExoticComponent<ComponentType<any>> => lazy(effect.componentImport);
