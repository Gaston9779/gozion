...
// ../../packages/react/dist/particles/react-bits/FaultyTerminal/FaultyTerminal.js
...
function FaultyTerminal({ scale = 1, gridMul = [2, 1], digitSize = 1.5, timeScale = 0.3, pause = false, scanlineIntensity = 0.3, glitchAmount = 1, flickerAmount = 1, noiseAmp = 1, chromaticAberration = 0, dither = 0, curvature = 0.2, tint = "#ffffff", mouseReact = true, mouseStrength = 0.2, dpr = Math.min(window.devicePixelRatio || 1, 2), pageLoadAnimation = true, brightness = 1, className, style, ...rest }) {
  ...
  const smoothMouseRef = (0, import_react.useRef)({ x: 0.5, y: 0.5 });
  const frozenTimeRef = (0, import_react.useRef)(0);
  ...
  (0, import_react.useEffect)(() => {
    ...
    const update = (t) => {
      ...
      if (!pause) {
        const elapsed = (t * 1e-3 + timeOffsetRef.current) * timeScale;
  ...
  return (0, import_jsx_runtime.jsx)("div", { ref: containerRef, className: `faulty-terminal-container ${className}`, style, ...rest });
}
