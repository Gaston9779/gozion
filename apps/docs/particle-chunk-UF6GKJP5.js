import {
  ...
  SMAAEffect,
  SMAAPreset
} from "./particle-chunk-GPZCQM4I.js";
import {
  Clock,
  Color,
  DoubleSide,
...
// ../../packages/react/dist/particles/react-bits/Hyperspeed/Hyperspeed.js
...
var distortions = {
  mountainDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const distortion = new Vector3(Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(movementProgressFix * Math.PI * uFreq.x + time) * uAmp.x, nsin(progress * Math.PI * uFreq.y + time) * uAmp.y - nsin(movementProgressFix * Math.PI * uFreq.y + time) * uAmp.y, nsin(progress * Math.PI * uFreq.z + time) * uAmp.z - nsin(movementProgressFix * Math.PI * uFreq.z + time) * uAmp.z);
  ...
  xyDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const distortion = new Vector3(Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.cos(movementProgressFix * Math.PI * uFreq.x + time) * uAmp.x, Math.sin(progress * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y - Math.sin(movementProgressFix * Math.PI * uFreq.y + time + Math.PI / 2) * uAmp.y, 0);
  ...
  LongRaceDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const distortion = new Vector3(Math.sin(progress * Math.PI * uFreq.x + time) * uAmp.x - Math.sin(camProgress * Math.PI * uFreq.x + time) * uAmp.x, Math.sin(progress * Math.PI * uFreq.y + time) * uAmp.y - Math.sin(camProgress * Math.PI * uFreq.y + time) * uAmp.y, 0);
  ...
  turbulentDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const distortion = new Vector3(getX(progress) - getX(progress + 7e-3), getY(progress) - getY(progress + 7e-3), 0);
  ...
  deepDistortion: {
    ...
    getJS: (progress, time) => {
      ...
      const distortion = new Vector3(getX(progress) - getX(progress + 0.01), getY(progress) - getY(progress + 0.01), 0);
...
function lerp(current, target, speed = 0.1, limit = 1e-3) {
...
var CarLights = class {
  ...
  init() {
    ...
    material.onBeforeCompile = (shader) => {
      shader.vertexShader = shader.vertexShader.replace("#include <getDistortion_vertex>", typeof this.options.distortion === "object" ? this.options.distortion.getDistortion : "");
    };
...
var Road = class {
  constructor(webgl, options) {
    ...
    __publicField(this, "leftRoadWay");
    __publicField(this, "rightRoadWay");
    __publicField(this, "island");
    this.webgl = webgl;
    this.options = options;
    this.uTime = { value: 0 };
  }
  createPlane(side, width, isRoad) {
    const options = this.options;
    const segments = 100;
    const geometry = new PlaneGeometry(isRoad ? options.roadWidth : options.islandWidth, options.length, 20, segments);
    let uniforms = {
      uTravelLength: { value: options.length },
      uColor: {
        value: new Color(isRoad ? options.colors.roadColor : options.colors.islandColor)
      },
      uTime: this.uTime
    };
    if (isRoad) {
...
var App = class {
  constructor(container, options) {
    ...
    __publicField(this, "leftCarLights");
    __publicField(this, "rightCarLights");
    __publicField(this, "leftSticks");
    __publicField(this, "fogUniforms");
    __publicField(this, "fovTarget");
    __publicField(this, "speedUpTarget");
    __publicField(this, "speedUp");
    __publicField(this, "timeOffset");
    __publicField(this, "hasValidSize");
    this.options = options;
    ...
    this.camera = new PerspectiveCamera(options.fov, initW / initH, 0.1, 1e4);
    ...
    this.leftCarLights = new CarLights(this, options, options.colors.leftCars, options.movingAwaySpeed, new Vector2(0, 1 - options.carLightsFade));
    this.rightCarLights = new CarLights(this, options, options.colors.rightCars, options.movingCloserSpeed, new Vector2(1, 0 + options.carLightsFade));
  ...
  update(delta) {
    ...
    this.speedUp += lerp(this.speedUp, this.speedUpTarget, lerpPercentage, 1e-5);
    ...
    if (typeof this.options.distortion === "object" && this.options.distortion.getJS) {
      ...
      this.camera.lookAt(new Vector3(this.camera.position.x + distortion.x, this.camera.position.y + distortion.y, this.camera.position.z + distortion.z));
      updateCamera = true;
    }
    if (updateCamera) {
      this.camera.updateProjectionMatrix();
...
var DEFAULT_EFFECT_OPTIONS = {};
var Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  const hyperspeed = (0, import_react.useRef)(null);
  const appRef = (0, import_react.useRef)(null);
  (0, import_react.useEffect)(() => {
    if (appRef.current) {
      appRef.current.dispose();
      appRef.current = null;
      const container2 = hyperspeed.current;
      if (container2) {
  ...
  return (0, import_jsx_runtime.jsx)("div", { id: "lights", ref: hyperspeed });
};
var Hyperspeed_default = Hyperspeed;
