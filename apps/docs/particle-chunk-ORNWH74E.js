...
// ../../packages/react/dist/particles/react-bits/PrismaticBurst/PrismaticBurst.js
...
var PrismaticBurst = ({ intensity = 2, speed = 0.5, animationType = "rotate3d", colors, distort = 0, paused = false, offset = { x: 0, y: 0 }, hoverDampness = 0, rayCount, mixBlendMode = "lighten" }) => {
  const containerRef = (0, import_react.useRef)(null);
  const programRef = (0, import_react.useRef)(null);
  const rendererRef = (0, import_react.useRef)(null);
  const mouseTargetRef = (0, import_react.useRef)([0.5, 0.5]);
  const mouseSmoothRef = (0, import_react.useRef)([0.5, 0.5]);
  const pausedRef = (0, import_react.useRef)(paused);
  const gradTexRef = (0, import_react.useRef)(null);
  const hoverDampRef = (0, import_react.useRef)(hoverDampness);
  const isVisibleRef = (0, import_react.useRef)(true);
  const meshRef = (0, import_react.useRef)(null);
  const triRef = (0, import_react.useRef)(null);
  ...
  (0, import_react.useEffect)(() => {
    ...
    let accumTime = 0;
    const update = (now) => {
      const dt = Math.max(0, now - last) * 1e-3;
      last = now;
  ...
  return (0, import_jsx_runtime.jsx)("div", { className: "prismatic-burst-container", ref: containerRef });
};
var PrismaticBurst_default = PrismaticBurst;

export {
  PrismaticBurst_default
};
