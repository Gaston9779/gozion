(() => {
  ...
  const copy = {
    it: {docs:'Documentazione',components:'Componenti',studio:'Tema',particles:'Particellari',search:'Cerca componenti',start:'Per iniziare',intro:'Introduzione',install:'Installazione',catalogue:'Catalogo',all:'Tutti i componenti',catalogueTitle:'Componenti che parlano da soli.',catalogueIntro:'Ogni anteprima mostra il vero scopo del componente ed è interattiva quando serve.',live:'Anteprima interattiva',usage:'Utilizzo',custom:'Personalizzazione',a11y:'Accessibilità',api:'API',copy:'Copia',copied:'Copiato',stable:'Stabile',beta:'Beta',preview:'Anteprima',open:'Apri',close:'Chiudi',disabled:'Disabilitato',size:'Dimensione',state:'Stato',variant:'Variante',density:'Densità',placement:'Posizione',language:'Lingua',theme:'Tema'},
    en: {docs:'Docs',components:'Components',studio:'Theme studio',particles:'Particles',search:'Search components',start:'Getting started',intro:'Introduction',install:'Installation',catalogue:'Catalogue',all:'All components',catalogueTitle:'Components that explain themselves.',catalogueIntro:'Every preview communicates the real purpose of its component and is interactive where it matters.',live:'Interactive preview',usage:'Usage',custom:'Customization',a11y:'Accessibility',api:'API',copy:'Copy',copied:'Copied',stable:'Stable',beta:'Beta',preview:'Preview',open:'Open',close:'Close',disabled:'Disabled',size:'Size',state:'State',variant:'Variant',density:'Density',placement:'Placement',language:'Language',theme:'Theme'}
  };
  ...
  function apiTable(name) {
    ...
    return `<table class="props-table"><thead><tr><th>Prop / slot</th><th>Type</th><th>Default</th><th>Description</th></tr></thead><tbody>${rows.map(([prop,type,defaultValue,desc]) => `<tr><td><code>${prop}</code></td><td>${type}</td><td>${defaultValue}</td><td>${desc}</td></tr>`).join('')}</tbody></table>`;
  }
  function renderSidebar() {
    ...
    sidebar.innerHTML = `${tree(t('start'), `<a class="side-link" href="#/getting-started">${t('intro')}</a><a class="side-link" href="#/installation">${t('install')}</a><a class="side-link" href="#/playground">${t('studio')}</a>`)}${tree(t('catalogue'), `<a class="side-link" href="#/components">${t('all')}</a>`)}<section class="side-collection"><h2>${t('components')}</h2>${componentGroups}</section><section class="side-collection"><h2>${t('particles')}</h2>${tree(state.locale === 'it' ? 'Backgrounds' : 'Backgrounds', particleLinks)}</section>`;
  }
  ...
  function renderCatalogue() {
    main.innerHTML = `<div class="crumbs"><a href="#/getting-started">${t('docs')}</a><span>/</span><span>${t('components')}</span></div><p class="eyebrow">${t('catalogue')}</p><h1 class="page-title">${t('catalogueTitle')}</h1><p class="page-intro">${items.length} ${t('catalogueIntro')}</p>${Object.entries(groups).map(([group,names]) => `<section><h2 class="section-title">${categoryLabel(group)}</h2><div class="component-grid">${names.map(name => componentCard(findComponent(slug(name)))).join('')}</div></section>`).join('')}`;
  ...
  function renderHome() {
    main.innerHTML = `<section class="hero hero-immersive"><div class="hero-particle" data-background-effect="aurora" data-hero-particle="aurora" aria-hidden="true"></div><div class="hero-particle hero-particle-orb" data-background-effect="orb" data-hero-particle="orb" aria-hidden="true"></div><div class="hero-content"><p class="eyebrow">Gozion UI / v0.2</p><h1 class="page-title">${state.locale === 'it' ? 'Un design system che resta tuo.' : 'A design system that stays yours.'}</h1><p class="page-intro">${state.locale === 'it' ? 'Componenti accessibili, token condivisi e API coerenti per React, Vue e Angular.' : 'Accessible components, shared tokens, and consistent APIs for React, Vue, and Angular.'}</p><div class="hero-actions"><a class="docs-button primary" href="#/components">${t('components')} →</a><a class="docs-button" href="#/particles">${t('particles')} →</a></div><div class="stat-row"><div class="stat"><strong>${items.length}</strong><span>${t('components').toLowerCase()}</span></div><div class="stat"><strong>3</strong><span>framework</span></div><div class="stat"><strong>53</strong><span>${t('particles').toLowerCase()}</span></div></div></div></section><h2 class="section-title">${state.locale === 'it' ? 'Inizia da qui' : 'Start here'}</h2><div class="component-grid particle-grid">${['Aurora','Orb','DotField','Waves'].map(name => componentCard(findComponent(slug(name)))).join('')}</div><h2 class="section-title">${t('components')}</h2><div class="component-grid">${['Button','Input','Card','Dialog','Switch','Table'].map(name => componentCard(findComponent(slug(name)))).join('')}</div>`;
    initBackgroundPreviews(main);
  }
