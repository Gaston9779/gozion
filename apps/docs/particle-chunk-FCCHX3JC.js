...
// ../../packages/react/dist/particles/react-bits/Ballpit/Ballpit.js
...
onResize_fn = function() {
  if (__privateGet(this, _resizeTimer))
    clearTimeout(__privateGet(this, _resizeTimer));
...
function createBallpit(canvas, config = {}) {
  ...
  initialize(config);
  spheres.update({ elapsed: 0, delta: 0 });
...
var Ballpit = ({ className = "", followCursor = true, ...props }) => {
  ...
  return (0, import_jsx_runtime.jsx)("canvas", { className, ref: canvasRef, style: { width: "100%", height: "100%" } });
};
